...
local a,b = file.Find("sound/*.*" , "GAME") 
for _, filename in pairs(a) do
	resource.AddFile("sound/"..string.lower(filename))
end
...
function GM:GetSlendermanSpawn()
	local spawns2 = ents.FindByClass("hostage_entity")
	
	if self.SlenderSpawn[game.GetMap()] == "cs_office" then
			return spawns2[math.random(1,#spawns2)]:GetPos()+vector_up*2
	end
...
function GM:CreateFlashLight()
	...
	if ent:IsValid() then
		...
		ent:SetKeyValue("lightfov", 60)//60
		ent:SetKeyValue("lightcolor", "205 205 205 125")
		ent:Spawn()
