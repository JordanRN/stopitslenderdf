...
function GM:PreDrawOpaqueRenderables()//HUDPaint()PostDrawOpaqueRenderables	
	...
	if scoreboard_alpha > 0 then
		...
		draw.SimpleText(math.random(40) == 1 and "NO DATA" or "Stop it, Slender! by NECROSSIN", "Tahoma_lines23",sX+sW-6,sY+4-23-6, Color(255,255,255,scoreboard_alpha), TEXT_ALIGN_RIGHT,TEXT_ALIGN_BOTTOM)
	...
	if VotingTime and VotingTime > CurTime() then
		...
		for ind = 1,#GAMEMODE.Maps do
			
			tbl = GAMEMODE.Maps[ind]
			
			draw.SimpleText(ind..".", "Tahoma_lines23",mX+6,mY+4+offsetY, Color(255,255,255,70), TEXT_ALIGN_LEFT,TEXT_ALIGN_BOTTOM)
...
local SpazzSounds = {
	[1] = {min = 401, max = 610, snd = { Sound("camera_static/single1.wav"), Sound("camera_static/single2.wav"), Sound("camera_static/single5.wav") } },
...
function GM:CheckVoting()
	...
	if VotingTime <= 0 then return end
	
	for key, ind in pairs(inputs) do
		if input.IsKeyDown( key ) and GAMEMODE.Maps[ind] and not Voted then
			RunConsoleCommand("vote_map",tostring(ind))
			Voted = true
			break
		end	
	end
